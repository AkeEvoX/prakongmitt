	...
	$.ajax({
	  ...
	  success: function(data) {
		...
		jQuery.each(JSON.parse(data.items),function(i,val){
				...
				row = "<tr id='"+val.id+"' >";
				row += "<td id='index' class='col-md-1'>"+val.id+"</td>";
				row += "<td id='name'>"+val.name+"</td>";
				row += "<td id='active' class='col-md-1' >"+status+"</td>";
				row += "<td id='icon'  class='col-md-1' ><img src='"+val.icon+"'  width='80' class='img-circle' /></td>";
...
function callInsert()
{
	popup('เพิ่มข้อมูลกลุ่มสินค้า','dialogs/productgroup.dialog.html',function(response,status,XMLHttpRequest){
			$('#saveitem').val('insert');
	});
}
...
function callsave(){
	...
	console.log('cateid = ' + dataSave["cateid"]);
	
		$.ajax({
		url:"controller/productgroup_manage.php",
		type:'POST',
...
function callupdate(obj){
	...
	popup('แก้ไขข้อมูลกลุ่มสินค้า','dialogs/productgroup.dialog.html', function(response,status,XMLHttpRequest) {
